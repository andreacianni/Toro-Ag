...
if ( ! function_exists('ta_render_video_prodotto_v2_shortcode') ) {
    function ta_render_video_prodotto_v2_shortcode($atts) {
        ...
        if ( ! function_exists('pods') ) {
            return '<!-- EN fix 8 --><!-- DEBUG: Pods non disponibile -->';
        }
        ...
        if ( ! $pod->exists() ) {
            return '<!-- EN fix 8 --><!-- DEBUG: Pod prodotto non trovato per ID ' . esc_html($prod_id_current) . ' -->';
        }

        // Recupera relazione video
        $videos = $pod->field('video_prodotto');
        $output .= '<!-- EN fix 8 --><!-- DEBUG: Raw video_prodotto: ' . esc_html(var_export($videos, true)) . ' -->';
        ...
        foreach ( (array) $videos as $item ) {
            ...
            if ( ! $video ) {
                $output .= '<!-- EN fix 8 --><!-- DEBUG: Video non trovato: ' . esc_html($video_id) . ' -->';
                continue;
            }

            $output .= '<!-- EN fix 8 --><!-- DEBUG: Titolo: ' . esc_html($video->post_title) . ' -->';
