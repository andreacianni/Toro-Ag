...
if ( ! function_exists('ta_render_video_prodotto_v2_shortcode') ) {
    function ta_render_video_prodotto_v2_shortcode($atts) {
        ...
        if ( ! function_exists('pods') ) {
            return '<!-- shortcode video_prodotto_v2 alpha 3 --><!-- DEBUG: Pods non disponibile -->';
        }
        ...
        $output .= '<!-- shortcode video_prodotto_v2 alpha 3 --><!-- DEBUG: Prodotto ID lingua ' . esc_html($current_lang) . ': ' . esc_html($prod_id_current) . ' -->';

        $pod = pods('prodotto', $prod_id_current, array('lang' => $current_lang));
        if ( ! $pod->exists() ) {
            return '<!-- shortcode video_prodotto_v2 alpha 3 --><!-- DEBUG: Pod prodotto non trovato per ID ' . esc_html($prod_id_current) . ' -->';
        }
        ...
        if ( empty($videos) ) {
            ...
            if ( empty($videos) ) return '<!-- shortcode video_prodotto_v2 alpha 3 --><!-- DEBUG: Nessun video -->';
        }
        ...
        foreach ( (array) $videos as $item ) {
            ...
            $output .= '<iframe title="' . esc_attr($video->post_title) . '" src="' . esc_url($video_link) . '" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>';
            $output .= '</div></div>';
