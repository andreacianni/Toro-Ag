...
function ta_render_video_prodotto_v2_shortcode($atts) {
    ...
    if ( ! function_exists('pods') ) {
        return '<!-- DEBUG: Pods non disponibile -->';
    }
    ...
    $output .= '<!-- DEBUG: Raw video_prodotto: ' . esc_html(var_export($videos, true)) . ' -->';
    ...
    if ( empty($videos) ) {
        ...
        if ( empty($videos) ) {
            $output .= '<!-- DEBUG: Ancora nessun video -->';
            return $output;
        }
    }
    ...
    foreach ( (array) $videos as $item ) {
        ...
        $output .= '<!-- DEBUG: ID raw: ' . esc_html($video_id) . ' -->';

        // Traduzione video
        $translated = apply_filters('wpml_object_id', $video_id, 'video', false, $current_lang);
        $video_id = $translated ? intval($translated) : $video_id;
        $output .= '<!-- DEBUG: ID tradotto: ' . esc_html($video_id) . ' -->';

        $video = get_post($video_id);
        if ( ! $video ) {
            $output .= '<!-- DEBUG: Video non trovato: ' . esc_html($video_id) . ' -->';
            continue;
        }

        $output .= '<!-- DEBUG: Titolo: ' . esc_html($video->post_title) . ' -->';
        $output .= '<div class="video-prodotto-item">';
        if ( has_post_thumbnail($video_id) ) {
            $output .= get_the_post_thumbnail($video_id, 'medium');
            $output .= '<!-- DEBUG: Thumbnail -->';
        }
        $output .= '<a href="' . esc_url(get_permalink($video_id)) . '">' . esc_html($video->post_title) . '</a>';
        $output .= '</div>';
    }
    $output .= '</div>';
