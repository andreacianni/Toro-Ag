...
if ( ! function_exists('ta_render_video_prodotto_v2_shortcode') ) {
    function ta_render_video_prodotto_v2_shortcode($atts) {
        ...
        if ( ! function_exists('pods') ) {
            return '<!-- shortcode video_prodotto_v2 alpha 2 --><!-- DEBUG: Pods non disponibile -->';
        }
        ...
        $output .= '<!-- shortcode video_prodotto_v2 alpha 2 --><!-- DEBUG: Prodotto ID lingua ' . esc_html($current_lang) . ': ' . esc_html($prod_id_current) . ' -->';

        // Carica Pod prodotto (lingua corrente)
        $pod = pods('prodotto', $prod_id_current, array('lang' => $current_lang));
        if ( ! $pod->exists() ) {
            return '<!-- shortcode video_prodotto_v2 alpha 2 --><!-- DEBUG: Pod prodotto non trovato per ID ' . esc_html($prod_id_current) . ' -->';
        }

        // Recupera relazione video
        $videos = $pod->field('video_prodotto');
        $output .= '<!-- shortcode video_prodotto_v2 alpha 2 --><!-- DEBUG: Raw video_prodotto: ' . esc_html(var_export($videos, true)) . ' -->';
        ...
        if ( empty($videos) ) {
            ...
            $output .= '<!-- shortcode video_prodotto_v2 alpha 2 --><!-- DEBUG: Fallback Prodotto IT ID: ' . esc_html($prod_id_default) . ' -->';

            $output .= '<!-- shortcode video_prodotto_v2 alpha 2 --><!-- DEBUG: Uso get_post_meta fallback multi -->';
            $videos_meta = get_post_meta($prod_id_default, 'video_prodotto', false);
            $output .= '<!-- shortcode video_prodotto_v2 alpha 2 --><!-- DEBUG: Raw get_post_meta fallback array: ' . esc_html(var_export($videos_meta, true)) . ' -->';
            $videos = array_map('intval', $videos_meta);
            $output .= '<!-- shortcode video_prodotto_v2 alpha 2 --><!-- DEBUG: Videos after map: ' . esc_html(var_export($videos, true)) . ' -->';

            if ( empty($videos) ) {
                $output .= '<!-- shortcode video_prodotto_v2 alpha 2 --><!-- DEBUG: Ancora nessun video -->';
                return $output;
            }
        }
        ...
        foreach ( (array) $videos as $item ) {
            ...
            $output .= '<!-- shortcode video_prodotto_v2 alpha 2 --><!-- DEBUG: ID raw: ' . esc_html($video_id) . ' -->';

            $translated = apply_filters('wpml_object_id', $video_id, 'video', true, $current_lang);
            $video_id = $translated ? intval($translated) : $video_id;
            $output .= '<!-- shortcode video_prodotto_v2 alpha 2 --><!-- DEBUG: ID tradotto: ' . esc_html($video_id) . ' -->';

            $video = get_post($video_id);
            if ( ! $video ) {
                $output .= '<!-- shortcode video_prodotto_v2 alpha 2 --><!-- DEBUG: Video non trovato: ' . esc_html($video_id) . ' -->';
                continue;
            }

            $lingua_terms = wp_get_post_terms($video_id, 'lingua_aggiuntiva', ['fields' => 'slugs']);
            $first_lingua = isset($lingua_terms[0]) ? $lingua_terms[0] : '';
            $output .= '<!-- shortcode video_prodotto_v2 alpha 2 --><!-- DEBUG: lingua_aggiuntiva slug: ' . esc_html($first_lingua) . ' -->';

            if ( ($current_lang === 'it' && $first_lingua !== 'italiano') || ($current_lang !== 'it' && $first_lingua === 'italiano') ) {
                $output .= '<!-- shortcode video_prodotto_v2 alpha 2 --><!-- DEBUG: Video escluso per lingua -->';
                continue;
            }

            $output .= '<!-- shortcode video_prodotto_v2 alpha 2 --><!-- DEBUG: Titolo: ' . esc_html($video->post_title) . ' -->';
            $output .= '<div class="video-prodotto-item">';
            if ( has_post_thumbnail($video_id) ) {
                $output .= get_the_post_thumbnail($video_id, 'medium');
                $output .= '<!-- shortcode video_prodotto_v2 alpha 2 --><!-- DEBUG: Thumbnail -->';
            }
            ...
            $flag_html = function_exists('toroag_get_flag_html') ? toroag_get_flag_html($first_lingua) : '';

            $output .= '<!-- shortcode video_prodotto_v2 alpha 2 --><!-- DEBUG: Link -->';
            $output .= '<a href="' . esc_url($video_url) . '">' . esc_html($video->post_title) . ' ' . $flag_html . '</a>';
            $output .= '<!-- shortcode video_prodotto_v2 alpha 2 --><!-- DEBUG: video_link: ' . esc_url($video_link) . ' -->';
            $output .= '<!-- shortcode video_prodotto_v2 alpha 2 --><!-- DEBUG: lingua_aggiuntiva: ' . esc_html($lingua_name) . ' -->';
