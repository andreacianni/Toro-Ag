...
if ( ! function_exists('ta_render_video_prodotto_v2_shortcode') ) {
    function ta_render_video_prodotto_v2_shortcode($atts) {
        ...
        if ( ! function_exists('pods') ) {
            return '<!-- shortcode video_prodotto_v2 alpha 4 --><!-- DEBUG: Pods non disponibile -->';
        }
        ...
        $output .= '<!-- shortcode video_prodotto_v2 alpha 4 --><!-- DEBUG: Prodotto ID lingua ' . esc_html($current_lang) . ': ' . esc_html($prod_id_current) . ' -->';

        $pod = pods('prodotto', $prod_id_current, array('lang' => $current_lang));
        if ( ! $pod->exists() ) {
            return '<!-- shortcode video_prodotto_v2 alpha 4 --><!-- DEBUG: Pod prodotto non trovato per ID ' . esc_html($prod_id_current) . ' -->';
        }
        ...
        if ( empty($videos) ) {
            ...
            if ( empty($videos) ) return '<!-- shortcode video_prodotto_v2 alpha 4 --><!-- DEBUG: Nessun video -->';
        }
        ...
        foreach ( (array) $videos as $item ) {
            ...
            $thumbnail = get_the_post_thumbnail_url($video_id, 'medium') ?: 'https://img.youtube.com/vi/' . basename($video_link) . '/mqdefault.jpg';
            ...
            $output .= '<a href="' . esc_url($video_link) . '" target="_blank" rel="noopener noreferrer">';
            $output .= '<div class="card-video embed-responsive embed-responsive-16by9">';
            $output .= '<img src="' . esc_url($thumbnail) . '" class="img-fluid" alt="Anteprima video">';
            $output .= '</div></a>';
