...
if ( function_exists( 'toroag_get_language_order' ) ) {
    $lang_order = toroag_get_language_order();
} else {
    $lang_order = [];
}
?>
...
<?php foreach ( $terms_data as $term ): ?>
                ...
                foreach ( $schede_by_lang as $lang_slug => $items ) : ?>
                  <div class="gruppo-lingua gruppo-lingua-<?php echo esc_attr( $lang_slug ); ?>">
                    <?php foreach ( $items as $item ) : ?>
                      <a href="<?= esc_url( $item['url'] ) ?>" target="_blank">
                        <span class="icone">
                          <?php echo toroag_get_flag_html( $lang_slug ); ?>
  ...
  <?php elseif ( $layout === 'grid' ): ?>
    <!-- GRID: griglia due colonne, prodotto / lingua+schede+documenti -->
    <ul class="list-group mb-5 documenti-download-list ps-0">
      ...
      <?php $index = 0; foreach ( $term['products'] as $prod ): ?>
        <?php
          $groups = [];
          foreach ( $prod['schede'] as $s ) { $groups[ $s['lang'] ]['schede'][] = $s; }
          foreach ( $prod['docs']  as $d ) { $groups[ $d['lang'] ]['docs'][]    = $d; }
          $total = count( $groups );
        ?>
        <li class="list-group-item d-flex">
          ...
          <div class="col-9 ps-0">
            <?php foreach ( $groups as $lang => $data ): $index++; ?>
              <div class="row<?php echo ( $index < $total ) ? ' border-bottom' : ''; ?> mb-0 pb-0 gruppo-lingua-<?php echo esc_attr( $lang ); ?> align-items-center">
